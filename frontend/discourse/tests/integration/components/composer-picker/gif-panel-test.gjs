...
module("Integration | Component | composer-picker/gif-panel", function (hooks) {
  ...
  hooks.beforeEach(function () {
    ...
    this.siteSettings.klipy_api_key = "test-key";
    this.siteSettings.klipy_country = "US";
    this.siteSettings.klipy_locale = "en_US";
    this.siteSettings.klipy_content_filter = "high";
  ...
  test("typing a query searches Klipy and renders results", async function (assert) {
    ...
    assert.true(
      ...
      "queries Klipy with the typed term"
    );
    assert
  ...
  test("fetches featured categories when opened", async function (assert) {
    ...
    assert.true(
      ...
      "requests featured categories with the api key on open"
    );
  });
